...
class Cell:
    ...
    def __init__(self, Spr_distance, Spr_len, *args, **kwargs ):
        self.spr_distance = Spr_distance
        self.spr_len = Spr_len
        self.spr_len_thickness = 0.02
        self.spr_coils = 8
        self.spr_radius = 0.08
        self.spr_color_vec = vec(144/255,238/255,144/255)
        self.opacity = 0.6
        self.lateral_opacity = 0.3
        self.centeral_opacity = 0.9
        self.ini_len = Spr_distance*0.01                      #ratio to be done
        self.max_len = Spr_distance*0.5 #1.8                     #ratio to be done
        self.max_dif_len = Spr_distance #2                  #ratio to be done
        self.module_num = 10
        ...
        x = 0
        y = 0
        x1 = x + self.spr_distance/2
        y1 = y - self.spr_distance/2 / np.sqrt(3)
        x2 = x - self.spr_distance/2
        y2 = y - self.spr_distance/2 / np.sqrt(3)
        x3 = x
        y3 = y + self.spr_distance/2 / np.sqrt(3) * 2


        x1 = x + self.spr_distance/2 / np.sqrt(3)
        y1 = y - self.spr_distance/2 

        x2 = x - self.spr_distance/2 / np.sqrt(3) * 2
        y2 = y 

        x3 = x + self.spr_distance/2 / np.sqrt(3)
        y3 = y + self.spr_distance/2 
        ...
        self.spring1[len(self.spring1):] = [helix(pos = self.walls[0],
                                                    axis = self.walls[1],
        ...
        self.spring2[len(self.spring2):]  = [helix(pos = self.walls[2],
                                                axis = self.walls[3],
                                                thickness = self.spr_len_thickness, coils = self.spr_coils,
                                                radius = self.spr_radius, color = self.spr_color_vec,
                                                opacity=self.lateral_opacity)]

        self.spring3[len(self.spring3):]  = [helix(pos = self.walls[4],
                                                axis = self.walls[5],
                                                thickness = self.spr_len_thickness, coils = self.spr_coils,
                                                radius = self.spr_radius, color = self.spr_color_vec,
                                                opacity=self.lateral_opacity)]
        ...
        self.Vec_12[len(self.Vec_12):] = [cylinder(pos = self.walls[0] + self.walls[1],
                                                      axis=vector.norm(self.walls[2] +self.walls[3] - (self.walls[0] + self.walls[1]))*self.spr_distance,
        ...
        self.Vec_23[len(self.Vec_23):] = [cylinder(pos = self.walls[2]  + self.walls[3],
                                                          axis=vector.norm(self.walls[4]+ self.walls[5]- (self.walls[2]+ self.walls[3])) * self.spr_distance,
                                                          radius=self.spr_len_thickness, color = self.spr_color_vec, opacity=self.opacity-0.55)]

        self.Vec_31[len(self.Vec_31):] = [cylinder(pos = self.walls[4] + self.walls[5],
                                                      axis=vector.norm(self.walls[0] + self.walls[1] - (self.walls[4] + self.walls[5])) * self.spr_distance,
                                                      radius=self.spr_len_thickness, color = self.spr_color_vec, opacity=self.opacity-0.55)]

        ring_rad = self.spr_distance/np.sqrt(3)
        self.top_cycle[len(self.top_cycle):] = [ring(pos = (self.Vec_12[len(self.Vec_12)-1].pos + self.Vec_23[len(self.Vec_23)-1].pos + self.Vec_31[len(self.Vec_31)-1].pos)/3,
                                                     axis = vector.cross(self.Vec_23[len(self.Vec_23)-1].axis, self.Vec_12[len(self.Vec_12)-1].axis),
    ...
    def update_top_segment(self):
        ...
        self.Vec_12[len(self.Vec_12) - 1].pos = self.walls[(Num_layer - 1)*6 + 0] + self.walls[(Num_layer - 1)*6 + 1]
        self.Vec_12[len(self.Vec_12) - 1].axis = vector.norm(self.walls[(Num_layer - 1)*6 + 2] + self.walls[(Num_layer - 1)*6 + 3] - (self.walls[(Num_layer - 1)*6 + 0] + self.walls[(Num_layer - 1)*6 + 1])) * self.spr_distance
        self.Vec_23[len(self.Vec_23) - 1].pos = self.walls[(Num_layer - 1)*6 + 2] + self.walls[(Num_layer - 1)*6 + 3]
        self.Vec_23[len(self.Vec_23) - 1].axis = vector.norm(self.walls[(Num_layer - 1)*6 + 4] + self.walls[(Num_layer - 1)*6 + 5] - (self.walls[(Num_layer - 1)*6 + 2] + self.walls[(Num_layer - 1)*6 + 3])) * self.spr_distance
        self.Vec_31[len(self.Vec_31) - 1].pos = self.walls[(Num_layer - 1)*6 + 4] + self.walls[(Num_layer - 1)*6 + 5]
        self.Vec_31[len(self.Vec_31) - 1].axis = vector.norm(self.walls[(Num_layer - 1)*6 + 0] + self.walls[(Num_layer - 1)*6 + 1] - (self.walls[(Num_layer - 1)*6 + 4] + self.walls[(Num_layer - 1)*6 + 5])) * self.spr_distance
        self.top_cycle[len(self.Vec_12) - 1].pos = (self.Vec_12[len(self.Vec_12)-1].pos + self.Vec_23[len(self.Vec_23)-1].pos + self.Vec_31[len(self.Vec_31)-1].pos)/3
        self.top_cycle[len(self.Vec_12) - 1].axis = vector.cross(self.Vec_23[len(self.Vec_23)-1].axis, self.Vec_12[len(self.Vec_12)-1].axis)
    ...
    def update_upper_layers(self, Layer_num_start):
        ...
        for i in range(Num_layer, Num_total):
            #print("start updatae layer:{0}".format(i))
    ...
    def update_current_layer_segment(self, Num_layer):
        ...
        self.Vec_12[Num_layer].axis = vector.norm(self.walls[(Num_layer)*6 + 2] + self.walls[(Num_layer)*6 + 3] - (self.walls[(Num_layer)*6 + 0] + self.walls[(Num_layer)*6 + 1])) * self.spr_distance
        self.Vec_23[Num_layer].pos  = self.walls[(Num_layer)*6 + 2] + self.walls[(Num_layer )*6 + 3]
        self.Vec_23[Num_layer].axis = vector.norm(self.walls[(Num_layer)*6 + 4] + self.walls[(Num_layer)*6 + 5] - (self.walls[(Num_layer)*6 + 2] + self.walls[(Num_layer)*6 + 3])) * self.spr_distance
        self.Vec_31[Num_layer].pos  = self.walls[(Num_layer)*6 + 4] + self.walls[(Num_layer )*6 + 5]
        self.Vec_31[Num_layer].axis = vector.norm(self.walls[(Num_layer)*6 + 0] + self.walls[(Num_layer)*6 + 1] - (self.walls[(Num_layer)*6 + 4] + self.walls[(Num_layer)*6 + 5])) * self.spr_distance

        self.top_cycle[Num_layer].pos = (self.Vec_12[Num_layer].pos + self.Vec_23[Num_layer].pos + self.Vec_31[Num_layer].pos)/3
    ...
    def add_one_layer_on_top(self):
        ...
        initial_vec = vector.cross((spring3_end - spring1_end), (spring2_end - spring1_end)).hat*self.ini_len
        ...
        self.Vec_12[len(self.Vec_12):] = [cylinder(pos = self.walls[(Num_layer_new - 1)*6 + 0] + self.walls[(Num_layer_new - 1)*6 + 1],
                                                      axis=vector.norm(self.walls[(Num_layer_new - 1)*6 + 2] +self.walls[(Num_layer_new - 1)*6 + 3] - (self.walls[(Num_layer_new - 1)*6 + 0] + self.walls[(Num_layer_new - 1)*6 + 1]))*self.spr_distance,
                                                      radius=self.spr_len_thickness, color = self.spr_color_vec, opacity=self.opacity-0.55)]

        self.Vec_23[len(self.Vec_23):] = [cylinder(pos = self.walls[(Num_layer_new - 1)*6 + 2]  + self.walls[(Num_layer_new - 1)*6 + 3],
                                                          axis=vector.norm(self.walls[(Num_layer_new - 1)*6 + 4]+ self.walls[(Num_layer_new - 1)*6 + 5] - (self.walls[(Num_layer_new - 1)*6 + 2]+ self.walls[(Num_layer_new - 1)*6 + 3])) * self.spr_distance,
                                                          radius=self.spr_len_thickness, color = self.spr_color_vec, opacity=self.opacity-0.55)]

        self.Vec_31[len(self.Vec_31):] = [cylinder(pos = self.walls[(Num_layer_new - 1)*6 + 4] + self.walls[(Num_layer_new - 1)*6 + 5],
                                                      axis=vector.norm(self.walls[(Num_layer_new - 1)*6 + 0] + self.walls[(Num_layer_new - 1)*6 + 1] - (self.walls[(Num_layer_new - 1)*6 + 4] + self.walls[(Num_layer_new - 1)*6 + 5])) * self.spr_distance,
                                                      radius=self.spr_len_thickness, color = self.spr_color_vec, opacity=self.opacity-0.55)]
    ...
    def increase_all_sides(self, stepLength):
        ...
        self.spring_len[(Num_layer - 1)] = [spring1_len + stepLength, spring2_len + stepLength, spring3_len + stepLength]
        self.update_top_segment()
        return 1
    ...
    def increasing_one_side_by_module(self, stepLength, SprNum, Module_Num):
        ...
        #print("start springN = {}, self.module_num={}".format(start_layerN+SprNum, self.module_num))
        for i in range(start_layerN+SprNum, start_layerN + self.module_num*3+ SprNum, 3):
    ...
    def change_inner_layer_len(self, LayerNum, spr1_len, spr2_len, spr3_len):
        ...
        list_axis = self.calculation_spring_axis(spr1_len, spr2_len, spr3_len, spring1_pos, spring2_pos, spring3_pos)
        if list_axis != 0:
    ...
    def change_inner_module_lens(self,Module_Num, len1, len2, len3 ):
        ...
        for i in range(Module_Num*self.module_num, (Module_Num + 1)*self.module_num ):
    ...
    def calculation_spring_axis(self, spring1_len, spring2_len, spring3_len,spring1_pos, spring2_pos, spring3_pos):
    ...
    def touchSense_parameters_cal(self, LayerNum, rodPos, rodEnd, rodRadius ):
        ...
        dist_center2Rod = calulation_line_to_point(up_center, rodPos, rodEnd) # - rodRadius - self.spr_distance*np.sqrt(3)/2
        dist_list = self.Distance_from_mid_layer_to_rod_shape(LayerNum, rodPos, rodEnd, rodRadius)
        angle0 = triangle_angle_from_3lines(dist_center2Rod, dist_list[0]  , self.spr_distance/np.sqrt(3))
        angle1 = triangle_angle_from_3lines(dist_center2Rod, dist_list[1]  , self.spr_distance/np.sqrt(3))
        angle2 = triangle_angle_from_3lines(dist_center2Rod, dist_list[2]  , self.spr_distance/np.sqrt(3))

        #angle = triangle_angle_from_3lines(dist_center2Rod, max(dist_list) + rodRadius , self.spr_distance/np.sqrt(3))
        print("touchSense_parameters_cal: angle = {}, {}, {}".format(np.rad2deg(angle0),np.rad2deg(angle1), np.rad2deg(angle2)))
...
def searching_coiling(cells, steplenght, rod_ending1, rod_ending2, rod_radius):
    ...
    for i in range(pre_moduleNum, total_module):
        ...
        layerNum,dist_spring = cells.touchSense_close_layer_point_by_module_from_new(new_module, rod_ending1, rod_ending2, rod_radius)
        [maxN, midN, minN] = find_len_order_from_list_from0(dist_spring)
        print("searching_coiling:dist_spring= {}, {}, {} ".format(dist_spring[0],dist_spring[1],dist_spring[2]))
        grow_adj = 0
        while (min(dist_spring) > 0.1 ):
...
def Grow_coiling(cells, steplenght, *args, **kwargs ):
    ...
    while(1):#np.min(dis_list) > 0.03 ):
        ...
        while( np.min(cells.Distance_from_top_to_target(*args, **kwargs))>=0.1 and np.min(cells.Distance_from_top_to_target(*args, **kwargs))<0.2  ):#max(dis_list) -  min(dis_list)> cells.spr_distance*0.85) : #to be done
...
def Grow_climbing(cells, steplenght, *args, **kwargs):
    ...
    while(1):#np.min(dis_list) > 0.03 ):
        ...
        while( np.min(cells.Distance_from_top_to_target(*args, **kwargs))>=0.1 and 
                ...
                max(dis_list) -  min(dis_list)> cells.spr_distance*0.85) : #to be done
            print("0.1 < min_dist < 0.2")
            dis_list = cells.Distance_from_top_to_target(*args, **kwargs)
            [maxN, midN, minN] = find_len_order_from_list(dis_list)
            res = cells.increase_all_sides(stepLength=steplenght * 0.2)
...
def Prepare_for_grasping(cells, steplenght, *args, **kwargs):
    ...
    while (max_dis - min_dis < cells.spr_distance*0.85):
...
def Show_coordinator_xyz(x, y ,z ):
    ...
    arrow_X = arrow(pos = coordinator_org, axis=vector(0.5,0,0), shaftwidth=0.02, color = color.white)
    arrow_Y = arrow(pos = coordinator_org, axis=vector(0,0.5,0), shaftwidth=0.02, color = color.white)
    arrow_Z = arrow(pos = coordinator_org, axis=vector(0,0,0.5), shaftwidth=0.02, color = color.white)
